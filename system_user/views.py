...
from merchant.models import MerchantBuyingRate
...
class ListSystemMerchant(viewsets.ModelViewSet):
    """
    ...
    def get_permissions(self):
        ...
        if self.action == 'create':  # POST request - No authentication needed for creating user
            return [AllowAny()]
        elif self.action in ['retrieve']:  # GET/PUT/PATCH actions require authentication
            return [IsAuthenticated()]
        elif self.action in ['update', 'partial_update']:  # GET/PUT/PATCH actions require authentication
    ...
    def get_queryset(self):
        ...
        """
        # Prefetch related MerchantBuyingRate objects for the merchant
        merchant_buying_rate_queryset = MerchantBuyingRate.objects.all()

        if self.action in ['retrieve', 'update', 'partial_update', 'destroy']:
            # Prefetch related MerchantBuyingRate for the particular merchant
            return CustomerInfo.objects.filter(user_type="MERCHANT") \
                .prefetch_related(Prefetch('merchantbuyingrate_set', queryset=merchant_buying_rate_queryset))

        elif self.action == 'list':
            # Only return all merchants if the requesting user is an admin
...
#         """
#         if self.action in ['retrieve', 'update', 'partial_update', 'destroy']:
#             return CustomerInfo.objects.filter(user_type="MERCHANT")  # Ensure only their own data
#         elif self.action == 'list':
#             # Only return all users if the requesting user is an admin
#             if self.request.user.is_staff:  # is_staff checks if the user is an admin
#                 return CustomerInfo.objects.filter(user_type="MERCHANT")  # Return all users for admin
#             return CustomerInfo.objects.filter(user_type="MERCHANT")  # Return no users if the user is not an admin
#         return CustomerInfo.objects.filter(user_type="MERCHANT")  # Return all users for other actions (create for example)